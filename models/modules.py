...
def st_block(x, Ks, channels, scope, keep_prob,is_training=True):
    '''
    ...
    with tf.variable_scope(f'stn_block_{scope}_temporal'):
        B, T, n, C = y.get_shape().as_list()
        with tf.variable_scope('temporal_embedding'):
            # x=tf.reshape(x,[-1,n,c_in])
            y = tf.transpose(y, [0, 2, 1, 3])
            y = tf.reshape(y, [-1, T, C])
            y = embedding_postprocessor(y,
                                          use_position_embeddings=True,
                                          position_embedding_name="temporal_position_embeddings",
                                          initializer_range=0.02,
                                          embeddings_length=T,
                                          dropout_prob=0.1)
        y = tf.reshape(y, [-1, n, T, C + 12])
        y = tf.transpose(y, [0, 2, 1, 3])
        x_o = temporal_transformer_layer(y, c_i, c_o, 1, is_training)
    #结合空间和时间
    x_combin = x_s + x_o
    x_ln = layer_norm(x_combin, f'layer_norm_{scope}')
    return x_ln
...
#
#
# def multi_head_attention(x, hidden_size, num_heads, mask):
#     depth = hidden_size // num_heads
#     wq = tf.layers.Dense(hidden_size)
#     wk = tf.layers.Dense(hidden_size)
#     wv = tf.layers.Dense(hidden_size)
#
#     q = wq(x)
#     k = wk(x)
#     v = wv(x)
...
#
#     attention_outputs = tf.split(attention_output, num_heads, axis=0)
#     concat_attention = tf.concat(attention_outputs, axis=-1)
#
#     output = tf.layers.Dense(hidden_size)(concat_attention)
#
#     return output
#
#
# def feed_forward_network(x, intermediate_size):
#     return tf.layers.Dense(intermediate_size, activation=tf.nn.relu)(x)
#
#
# def transformer_layer(x, hidden_size, num_heads, intermediate_size, mask):
#     attention_output = multi_head_attention(x, hidden_size, num_heads, mask)
#
#     # 线性变换，将 attention_output 的最后一个维度从 hidden_size 扩展到 intermediate_size
#     attention_output = tf.layers.dense(attention_output, units=intermediate_size, activation=None)
#
#     # 对 x 进行线性变换，将其最后一个维度从 hidden_size 扩展到 intermediate_size
#     x = tf.layers.dense(x, units=intermediate_size, activation=None)
#
#     # 执行残差连接和 Layer Normalization
#     residual = x + attention_output
#     normalized_residual = tf.contrib.layers.layer_norm(residual, begin_norm_axis=-1)
#
#     feed_forward_output = feed_forward_network(normalized_residual, intermediate_size)
#     layer_output = tf.contrib.layers.layer_norm(normalized_residual + feed_forward_output, begin_norm_axis=-1)
#
#     return layer_output
#
# def create_mask(seq):
#     # 生成一个上三角矩阵，将未来位置的值设为一个很大的负数
#     seq_len = tf.shape(seq)[1]
#     mask = 1 - tf.linalg.band_part(tf.ones((seq_len, seq_len)), -1, 0)
#     return mask
#
#
# def temporal_transformer_layer(x, hidden_size, num_heads, h, is_training=True):
#     print(f"使用了-------------->retem")
#     mask = None  # 这里你可能需要添加一个attention mask
#
#     with tf.variable_scope('encoder'):
#         t_transformer_output = transformer_layer(x, hidden_size, num_heads, hidden_size * 1, mask)
#
#     return tf.nn.relu(t_transformer_output)
#
#
# def transformer_decoder_layer(start_tensor, x, hidden_size, num_heads, is_training=True):
#     mask = None  # 这里你可能需要添加一个attention mask
#
#     with tf.variable_scope('decoder'):
#         attention_output = multi_head_attention(start_tensor, hidden_size, num_heads, mask)
#         add_and_norm1 = tf.contrib.layers.layer_norm(start_tensor + attention_output, begin_norm_axis=-1)
#
#         t_transformer_output = transformer_layer(add_and_norm1, hidden_size, num_heads, hidden_size * 4, mask)
#
#     return tf.nn.relu(t_transformer_output)
